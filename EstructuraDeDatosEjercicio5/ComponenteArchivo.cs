...
{
    internal static class ComponenteArchivo
    {
        ...
        */
        ...
        static List<ComponenteEnt> componentes;
        ...
        static ComponenteArchivo()
        {
            //si existe el archivo...
            if (File.Exists("componentes.json"))
            {
                //lee TODO el contenido del archivo.
                string contenidoDelArchivo = File.ReadAllText("componentes.json");

                //esta linea convierte el texto
                //de vuelta a objetos de tipo PersonaEnt;

                componentes = JsonConvert.DeserializeObject<List<ComponenteEnt>>(contenidoDelArchivo);
            }
            else
            {
                componentes = new List<ComponenteEnt>();
            }
        }

        //Estilo 1: este modulo devuelve una lista de todas las personas
        //y el resto del sistema trabaja con eso.
        public static List<ComponenteEnt> ObtenerComponentes()
        {
            return componentes.ToList();
        }


        public static void AgregarComponente(ComponenteEnt componente)
        {
        ...
        */
        ...
        {
            var contenido = JsonConvert.SerializeObject(componentes);
            File.WriteAllText("componentes.json", contenido);
        }
    }
}
